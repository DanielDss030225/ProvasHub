...
export default function SolvePage({ params }: PageProps) {
    ...
    const confirmAndNext = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        // Safety check: if no answer selected, it can't be correct
        ...
        // 2. Navigate to Next Question or Finish
    ...
    const handleFinish = async () => {
        ...
        if (rewardCoins > 0) {
            try {
                ...
                // Create a batch of coins with staggered delays
                ...
                // We don't reset rewardAwarded here because status is already finished,
                // and we want it to trigger on click, which only happens once usually.
            }
        }
    };
                    ...
                    <div className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 md:px-6 shadow-sm z-10 shrink-0 transition-colors gap-2">
                        <div className="flex items-center gap-2 md:gap-4 shrink-0">
                            ...
                            <div className="flex flex-col min-w-0 max-w-[120px] md:max-w-xs">
                                <h1 className="font-bold text-slate-800 dark:text-white leading-tight truncate text-sm md:text-base" title={exam.extractedData?.title || exam.fileName}>
