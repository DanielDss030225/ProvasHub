...
export interface Question {
  ...
  text: string;
  options: string[];
  correctAnswer?: string; // Optional, if AI can infer or if provided
  confidence: number;
  hasGraphic?: boolean;
}
...
function cleanAndParseJSON(text: string): any {
  ...
  // 1. Extract JSON object using regex (finds the first { and last })
  ...
  // 2. Use JSON5 for lenient parsing (handles trailing commas, unescaped chars, etc.)
  try {
...
export async function processExam(base64Data: string, mimeType: string = "application/pdf"): Promise<ExamData> {
      ...
      "questions": [
        {
          ...
          // If extracted, use single letter lowercase 'a', 'b', 'c', etc.
          "correctAnswer": "a", 
          "hasGraphic": true,
          "confidence": 0.95
        }
  ...
  try {
    ...
    console.error("Error processing exam inside processor:", error);
    throw error;
  }
}
